...
Class Image{
    ...
    public function __construct($args = array()) {
        ...
        $this->lcp_checked = isset($this->args["lcp"])?true:false;
        ...
        if($this->args["preview"]){
            ...
            $lcp_status = false;
            if($this->lcp_checked){
                $lcp_status = $this->args["lcp"];
            }else{
                $lcp_status = image_is_lcp($this->args['src']);
            }
            if($lcp_status){
                $this->args["lcp"] = true;
                $this->args["lazy"] = false;
    ...
    public function generateMediaQueries($selected) {
        ...
        $last = end($selected);
        $queries = [];
        $breakpoints = array_reverse($this->breakpoints);
        //print_r($selected);
    ...
    public function get_image_set_multiple($args=array(), $has_breakpoints = false){
        if(!$has_breakpoints){
            ...
            foreach ($args as $index => $item) {
                ...
                $best_image = $this->find_best_image_for_breakpoint($args, $breakpoint, array_keys($breakpoints));
