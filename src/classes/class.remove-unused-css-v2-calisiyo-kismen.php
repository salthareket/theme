...
class RemoveUnusedCss {
    ...
    */
    ...
    public function __construct($html, $css, $output = "", $additional_whitelist = [], $critical_css = false, array $opts = []) {
        ...
        error_log("[RemoveUnusedCss] --------------------------------------");
        error_log("[RemoveUnusedCss] --------------------------------------");
        error_log("[RemoveUnusedCss] Starting............................. ");
        error_log("[RemoveUnusedCss] --------------------------------------");
        error_log("[RemoveUnusedCss] --------------------------------------");
...
private function checkMediaQueries($dom) {
    if (empty($this->css_structure["media_queries"])) return;
    ...
    // Senin orijinal sıralama mantığın (Burası hatasız çalışıyor)
    ...
    foreach ($mqItems as $item) {
        ...
        $rules_in_media = $item['rules']; // Bu bir array: ["selector" => "code"]
        $final_media_css_block = '';

        if (!is_array($rules_in_media)) continue;

        foreach ($rules_in_media as $selector_group => $style_code) {
            // Seçicileri virgülle ayır (örn: .typeahead, .nav)
            ...
            foreach ($all_selectors_in_group as $individual_selector) {
                ...
                // 1. Blacklist
                if ($this->isBlacklisted($individual_selector)) continue;

                // 2. Whitelist (ignore_whitelist kapalıysa)
                if (!($this->opts['ignore_whitelist'] ?? false)) {
                    if ($this->isWhitelisted($dom, $individual_selector)) {
                        $keptSelectors[] = $individual_selector;
                        continue;
                    }
                }

                // 3. ZİNCİRLEME KONTROL (Beyin burası)
                // Senin .typeahead.dropdown-menu burada sorgulanıyor
                if ($this->selectorExists($dom, $individual_selector)) {
                    $keptSelectors[] = $individual_selector;
                    error_log("+++ EXIST: ".$individual_selector);
                }else{
                    error_log("--- NOT EXIST: ".$individual_selector);
                }
            }

            // Sadece yaşayan seçicileri ekle
            if (!empty($keptSelectors)) {
                $final_media_css_block .= implode(", ", $keptSelectors) . " { " . $style_code . " }\n";
            }
        }

        // Eğer media query'nin içi doluysa ana çıktıya bas
...
private function processStyles($dom) {
    foreach ($this->css_structure["styles"] as $style) {
        if (!is_array($style) || !isset($style["selectors"])) continue;
        $all_selectors_in_group = $style["selectors"];
        $keptSelectors = [];

        foreach ($all_selectors_in_group as $individual_selector) {
            $individual_selector = trim($individual_selector);
            if (empty($individual_selector)) continue;

            // Blacklist'teyse direkt ele
            if ($this->isBlacklisted($individual_selector)) continue;

            // TAM SELECTOR (örn: .card.slider > .title) fonksiyona gider
            if ($this->selectorExists($dom, $individual_selector)) {
                $keptSelectors[] = $individual_selector;
                error_log("+++ EXIST: ".$individual_selector);
            }else{
        ...
        // Sadece hayatta kalan selector'lar varsa bloğu inşa et
        if (!empty($keptSelectors)) {
...
private function getStaticBase($part) {
    $static = $part;
    foreach ($this->white_list as $white) {
        $quoted = preg_quote($white, '/');
        $pattern = str_replace('\*', '[a-zA-Z0-9\-_]*', $quoted);
        // Sadece tam sınıf eşleşmelerini temizle (Örn: .fa-solid'i sil ama .fa-trash'e dokunma)
        $static = preg_replace('/' . $pattern . '(?!\w)/i', '', $static);
    }
    $static = trim($static, ' .#');
    if (empty($static)) return '';
    $prefix = (strpos($part, '.') === 0) ? '.' : ((strpos($part, '#') === 0) ? '#' : '');
    return $prefix . $static;
}

public function selectorExists($dom, $selector) {
    $selector = trim($selector);
    if (empty($selector)) return false;

    // 1. Temel Etiketler
    if (preg_match('/^(html|body|\*)$/i', $selector)) return true;

    // 2. Pseudo-element temizliği (Arama için)
    $cleanSelector = preg_replace('/(?<!:):{1,2}[a-zA-Z0-9_-]+(\([^)]*\))?/i', '', $selector);
    $cleanSelector = trim($cleanSelector);
    if (empty($cleanSelector)) return true;

    try {
        // Seçiciyi parçalara ayır (space, >, +, ~)
        $parts = preg_split('/\s*([\s>+~])\s*(?![^\[]*\])/', $cleanSelector, -1, PREG_SPLIT_DELIM_CAPTURE);
        ...
        for ($i = 0; $i < count($parts); $i++) {
            ...
            // --- YENİ MANTIK: WHITELIST SADECE TEKİL SINIFLARDA GEÇERLİ ---
            // Eğer parça ".fa-trash.fa-solid" gibi bileşikse, whitelist'e güvenmiyoruz, DOM'a bakıyoruz.
            $isCompound = (substr_count($part, '.') + substr_count($part, '#')) > 1;

            if (!$isCompound && $this->isWhitelisted($dom, $part)) {
                return true; 
            }

            $next_matches = [];
            // Bileşik sınıfın içinden whitelist olanları temizleyip ana gövdeyi buluyoruz
            // Örn: .fa-trash.fa-solid -> .fa-trash kalır
            $staticPart = $this->getStaticBase($part);
            $searchQuery = !empty($staticPart) ? $staticPart : $part;

            foreach ($contexts as $ctx) {
                try {
                    $found = $ctx->find($searchQuery);
                    if ($found) {
                        foreach ($found as $f) $next_matches[] = $f;
                    }
                } catch (\Exception $e) { continue; }
            }

            // Statik parça (yani ikonun kendisi) DOM'da yoksa bitti, siliyoruz.
            if (empty($next_matches)) {
                return false; 
            }

            $contexts = $next_matches;
        }
...
private function isWhitelisted($dom, $selector) {
    if ($this->options['ignore_whitelist'] ?? false) return false;
    foreach ($this->white_list as $white) {
        $quoted = preg_quote($white, '/');
        $pattern = '/^' . str_replace('\*', '[a-zA-Z0-9_-]*', $quoted) . '$/i';
        if (preg_match($pattern, $selector)) return true;
    }
    return false;
}
