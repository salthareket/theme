...
class RemoveUnusedCss {
    ...
    */
    ...
    /**
     * Seçiciden, XPath hatalarına neden olan Sözde Öğeleri ve desteklenmeyen Sözde Sınıfları kaldırır.
     *
     * @param string $selector
     * @return string Yalnızca DOM sorgusuna uygun seçiciyi döndürür.
     */
    private function stripUnsupportedSelectors_v1(string $selector): string
    {
        // Sözde Öğeleri Kaldırma:
        // :: ile başlayan her şeyi kaldır, ancak bu seçicilerin varlığını KORUMAK için
        // selectorExists'ta bu seçiciyi daima true olarak döndürmeliyiz.
        // Şimdilik sadece bu kısmı DOM sorgusundan kaldırıyoruz.
        
        // a) Genel Sözde Öğeler: :: ile başlayan her şeyi kaldır
        $selector = preg_replace('/::[a-zA-Z0-9\-\(\)]+/', '', $selector);

        // b) Tek tırnakla (eski stil) tanımlanmış ve hata veren kritik Sözde Öğeler (DOM'da bulunmazlar):
        // :before, :after, :placeholder'ı tek iki nokta üst üste ile yazılmış olsa da kaldır.
        // Eğer cleanSelector'da bunları koruyorsanız, burada tekrar kaldırabilirsiniz.
        // Not: cleanSelector zaten sadece kabul edilenleri tutuyor, bu yüzden burada kalanları DOM'a uygun hale getirelim.
        
        // :not() içindeki seçiciyi bozmamak için daha dikkatli regex kullanın.
        // Örneğin, sadece seçicinin en sonunda veya bir boşluktan sonra gelen pseudo'ları hedefleyin:
        $selector = preg_replace('/(?:^|[^a-zA-Z0-9\-\)\.])(:before|:after|:placeholder)/i', '$1', $selector);

        // cleanSelector'ın yaptığı:
        // Eğer cleanSelector sadece izin verilenleri koruyorsa, buraya gelen seçici zaten filtrelenmiştir.
        // Fakat :hover, :focus gibi kabul edilenler de DOM'da bulunamayabilir.
        
        // En Güvenli Yaklaşım (Selector'ı DOM'a Uygun Hale Getirme):
        // Sözde sınıfları ve öğeleri, DOM sorgusuna uygun kalacak şekilde temizleyin (parantez içindekiler hariç).
        // :nth-child(2n+1) gibi XPath'e çevrilebilenleri KORUYUN.
        // :hover, :focus, :before, ::after gibi DOM'a çevrilemeyenleri KALDIRIN.
        
        // Sözde Öğeleri ve DOM sorgusunu bozan Sözde Sınıfları kaldır:
        $selector = preg_replace('/\s*::[a-zA-Z0-9\-\(\)]+/', '', $selector); // ::after, ::before
        $selector = preg_replace('/:focus-within/i', '', $selector); // Bazen sorun çıkarır
        $selector = preg_replace('/:hover/i', '', $selector); // DOM sorgusuna uygun değil
        $selector = preg_replace('/:active/i', '', $selector); // DOM sorgusuna uygun değil
        // ... Sizin cleanSelector'da koruduğunuz ve XPath'e çevrilemeyen her şeyi buraya ekleyin.
    ...
    /**
     * Seçiciden, XPath hatalarına neden olan Sözde Öğeleri ve desteklenmeyen Sözde Sınıfları kaldırır.
     * Bu, tüm fonksiyonel pseudo-class'ları ve geride kalan operatör/sayı artıklarını siler.
     *
     * @param string $selector
     * @return string Yalnızca DOM sorgusuna uygun seçiciyi döndürür.
     */
    private function stripUnsupportedSelectors(string $selector): string
    {
        // 1. cleanSelector'ı çağırarak kabul edilmeyen pseudo'ları temizle
        // (Bu metotun mevcut kodunuzda var olduğunu varsayıyoruz.)
        $selector = $this->cleanSelector($selector);

        // 2. KRİTİK AGRESİF TEMİZLİK: Fonksiyonel pseudo-class'ları tamamen sil.
        // n+3 hatasını ve not() parantez hatalarını çözer.
        // Pattern: Colondan (:) başlayıp, parantezleri ve içindeki içeriği yakalayıp siler.
        // Not: [^)]* yerine `[^)]*?` kullanıldı, parantez içindeki tırnakları ve diğer karmaşık yapıları daha iyi temizlemek için.
        $selector = preg_replace('/:[a-zA-Z0-9\-]+\s*\([^)]*\)/', '', $selector);

        // 3. Pseudo Elementleri (::before, ::after) ve Tek Kolonlu Dinamikleri Sil.
        $selector = preg_replace('/\s*::[a-zA-Z0-9\-\(\)]+/', '', $selector);
        $selector = preg_replace('/:before\b/i', '', $selector);
        $selector = preg_replace('/:after\b/i', '', $selector);
        $selector = preg_replace('/:placeholder\b/i', '', $selector);
        $selector = preg_replace('/::placeholder\b/i', '', $selector);
        $selector = preg_replace('/:focus-within/i', '', $selector); 
        $selector = preg_replace('/:hover/i', '', $selector);
        $selector = preg_replace('/:active/i', '', $selector);
        
        // 4. NİHAİ ARTIK TEMİZLİĞİ: Geriye kalan anlamsız karakterleri kaldır.
        
        // A) Seçici sonunda kalan kolonları temizle (örn: `.class:`)
        $selector = preg_replace('/:\s*$/', '', $selector); 
        
        // B) Yanlışlıkla geriye kalan Operatör + Sayı kalıbını tamamen sil (Örn: `tr+3` -> `tr`)
        // Bu, son hatayı çözen ana adımdır.
        $cleaned = preg_replace('/([>+~])\s*([\+\-]?\d+)/', '', $selector); 

        // C) Kalan tüm parantezleri sil
        $cleaned = str_replace(['(', ')'], '', $cleaned);
        
        // D) Fazla boşlukları ve gereksiz operatörleri temizle
        $cleaned = preg_replace('/\s+/', ' ', $cleaned);
        $cleaned = preg_replace('/\s*([>+~])\s*/', '$1', $cleaned);
        
        // Seçicinin başında/sonunda kalan operatörleri temizle
    ...
    private function checkMediaQueries_v1($dom) {
    ...
    private function checkMediaQueries($dom) {
        // 1) Sırala (küçükten büyüğe)
        // ... (Mevcut sıralama mantığı korundu)
        $mqItems = [];
        foreach ($this->css_structure["media_queries"] as $mq => $rules) {
            $mqItems[] = ['meta' => $this->parseMediaForSort($mq), 'rules' => $rules];
        }
        ...
        foreach ($mqItems as $item) {
            ...
            // Media içindeki her selector grubunu filtrele
            foreach ($rules_in_media as $selector_group => $style_code) {
                // Seçici grubunu (virgüllerle ayrılmış) korumalı olarak böl
                $all_selectors_in_group = $this->split_selectors_respecting_brackets($selector_group);
                $keptSelectors = [];

                foreach ($all_selectors_in_group as $individual_selector) {
                    if ($individual_selector === '') continue;

                    // 1. Durum: Whitelist/Blacklist kontrolü (öncelikli)
                    if ($this->isBlacklisted($individual_selector)) {
                        continue; // Blacklist'te ise atla
                    }
                    
                    // 2. Durum: Temizlenmiş Seçicinin DOM'da varlığını kontrol et
                    // Artık selectorExists, tüm parser hatalarını içeriden yönetmektedir.
                    if ($this->selectorExists($dom, $individual_selector)) {
                        $keptSelectors[] = $individual_selector;
                        continue;
                    }
    ...
    /**
     * Verilen seçiciden tüm .class ve #id'leri ayıklar.
     */
    private function extractAllClassAndIdSelectors(string $selector): array
    {
        $components = [];
        // Tüm class'ları (. ile başlayan) ve ID'leri (# ile başlayan) yakala
    ...
    /**
     * Seçicinin herhangi bir .class veya #id bileşeni beyaz listede mevcutsa TRUE döner.
     * Bu, TWIG/JS ile dinamik olarak eklenen class'ların silinmesini engeller.
     */
    private function isAnySelectorComponentWhitelisted(string $selector): bool
    {
        // 1. Seçiciden tüm class ve ID'leri ayıkla (Örn: .modal.is-open -> [.modal, .is-open])
        $components = $this->extractAllClassAndIdSelectors($selector);
        
        if (empty($components)) {
            // Hiç class veya ID yoksa, bu bir tag veya pseudo kuralıdır, normal akış devam etmeli.
            return false;
        }

        foreach ($components as $component) {
            foreach ($this->white_list as $whitelist_item) {
                // Wildcard kontrolü (örn: .fixed-*, .loading-*)
                $is_wildcard = strpos($whitelist_item, '*') !== false;
                
                if ($is_wildcard) {
                    $pattern = str_replace('*', '.*', preg_quote($whitelist_item, '/'));
                    // Baştan sona tam eşleşme
                    if (preg_match('/^' . $pattern . '$/', $component)) {
                        return true;
                    }
                } else {
                    // Tam eşleşme
                    if ($component === $whitelist_item) {
                        return true;
                    }
                }
            }
    ...
    private function selectorExists_v1($dom, $selector) {
    ...
    private function selectorExists($dom, $selector) {
        ...
        // 1. DYNAMIC/UNIVERSAL SELECTOR KONTROLÜ
        if ($selector === '*' || preg_match('/^::?(before|after)\b/i', $selector) || preg_match('/^:root\b/i', $selector)) {
            return true;
        }
        
        // ✅ KRİTİK DÜZELTME: FAZLADAN SİLİNMEYİ ENGELLEME
        // Seçicinin herhangi bir class veya ID bileşeni beyaz listede varsa, DOM sorgusunu atla ve KORU.
        // Bu, TWIG'den gelen ve DOM parser'ın görmekte zorlandığı class'ları korur.
        if ($this->isAnySelectorComponentWhitelisted($selector)) {
            return true;
        }
        // --------------------------------------------------------

        // 2. DOM SORGUSU İÇİN SEÇİCİYİ TEMİZLE (Önceki hataları önlemek için agresif temizlik)
        $dom_query_selector = $this->stripUnsupportedSelectors($selector);

        // KRİTİK HATA DÜZELTMESİ: Temizlenmiş seçici, DOM sorgusu için geçerli değilse durdur.
        $dom_query_selector = trim($dom_query_selector);
        
        // Boş veya sadece anlamsız bir operatör/kolondan oluşanları engelle
        if (
            empty($dom_query_selector) || 
            $dom_query_selector === ':' || 
            preg_match('/^[>+~]$/', $dom_query_selector) 
        ) {
            return true; // Kuralı koru
        }
        
        // 3. ASIL DOM SORGUSU
        $elements = $dom->find($dom_query_selector); 
        if (!$elements || count($elements) === 0) {
            return false;
        }else{
            return true;
