...
function admin_plugins_activated($plugin, $network_activation) {
    if ($plugin === 'woocommerce/woocommerce.php') {
        ...
        $woo_pages = [
            ...
            ['endpoint' => 'cart',           'title' => 'Cart',     'content' => '<!-- wp:woocommerce/cart --><div class="wp-block-woocommerce-cart is-loading"></div><!-- /wp:woocommerce/cart -->',     'template' => ''],
            ['endpoint' => 'checkout',       'title' => 'Checkout', 'content' => '<!-- wp:woocommerce/checkout --><div class="wp-block-woocommerce-checkout is-loading"></div><!-- /wp:woocommerce/checkout -->', 'template' => ''],
...
function salt_enforce_page_shortcodes($post_id, $post, $update) {
    ...
    // Shortcode gerektiren sayfa ID -> shortcode eslesmesi
    $is_woo = class_exists('WooCommerce');
    $pages = [];
    ...
    if ($myaccount_id) {
        $pages[$myaccount_id] = $is_woo ? '[woocommerce_my_account]' : '[salt_my_account]';
    }

    // WooCommerce sayfalari
    if ($is_woo) {
        $wc_pages = [
        ...
        foreach ($wc_pages as $endpoint => $block_content) {
            $pid = (int) get_option('woocommerce_' . $endpoint . '_page_id');
            if ($pid) $pages[$pid] = $block_content;
        }
    }

    // Bu sayfa kontrol edilmesi gereken sayfalardan biri mi?
    if (!isset($pages[$post_id])) return;

    $required_content = $pages[$post_id];
    $content = $post->post_content;

    // Gerekli icerik zaten var mi?
    // Block icerikleri icin block comment'ini, shortcode icin shortcode'u ara
    if (strpos($required_content, 'wp:woocommerce/') !== false) {
        // Block bazli — block comment var mi kontrol et
        $block_name = '';
        if (preg_match('/wp:woocommerce\/(\w+)/', $required_content, $m)) {
            $block_name = $m[1];
        }
        if ($block_name && strpos($content, 'wp:woocommerce/' . $block_name) !== false) return;
        // Eski shortcode varsa onu da kabul et (legacy uyumluluk)
        $legacy_shortcode = '[woocommerce_' . $block_name . ']';
        if (strpos($content, $legacy_shortcode) !== false) return;
    } else {
        // Shortcode bazli
        $shortcode_tag = '';
        ...
        if ($shortcode_tag && strpos($content, '[' . $shortcode_tag . ']') !== false) return;
    }

    // Yoksa ekle
    if (empty(trim($content))) {
        $new_content = $required_content;
    } else {
        $new_content = $required_content . "\n" . $content;
    }

    // Sonsuz donguyu engelle
