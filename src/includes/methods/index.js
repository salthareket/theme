...
ajax_hooks['login'] = {required:"bootbox",before:function(response,vars,form,objs){console.log(response)
if(!form){form=objs.form}
if(form.length){form.addClass("loading-process")}},after:function(response,vars,form,objs){console.log(response)
if(!form){form=objs.form}
if(form.length){form.removeClass("loading-process")}
response_view(response)}};
ajax_hooks['get_floors'] = {before:function(response,vars,form,objs){$("body").addClass("loading")},after:function(response,vars,form,objs){$(".search-result-text").html(response.data);$(".container-floors").addClass("search-results").html(response.html);$("body").removeClass("loading");store_modal_link()}};
ajax_hooks['get_store_modal'] = {before:function(response,vars,form,objs){var dialog=bootbox.dialog({className:"modal-store page-store modal-fullscreen- loading",message:'<div></div>',closeButton:!1,backdrop:!0,size:'xl',buttons:{},onHidden:function(e){response.abort()}})},after:function(response,vars,form,objs){twig({href:ajax_request_vars.theme_url+"theme/templates/magazalar/single-modal.twig",async:!1,allowInlineIncludes:!0,load:function(template){let data=window["store_"+vars.store_id];console.log(vars)
console.log(data)
...
ajax_hooks['get_stories'] = {before:function(response,vars,form,objs){$("#"+objs.story).parent().addClass("loading loading-xs")},after:function(response,vars,form,objs){console.log(response,vars,form,objs);var story_args={backNative:!1,backButton:!0,previousTap:!1,skin:'snapgram',autoFullScreen:!1,avatars:!0,paginationArrows:!1,list:!1,openEffect:!1,cubeEffect:!0,localStorage:!0,reactive:!1,rtl:root.lang=="ar"?true:!1,stories:response.stories,language:response.language,callbacks:{onOpen(storyId,callback){if(typeof storyId==="undefined"){if($("#zuck-modal").is(":visible")){console.log("instagram sttory exist")
$("#zuck-modal").css("display","none").find("#zuck-modal-content").empty()}}
callback();console.log("onOpen",storyId,callback);$("[data-story-id='"+storyId+"']").unbind("click mousedown touchstart");$("#zuck-modal-slider-stories-main").unbind("mousedown touchstart");$("body").unbind("click mousedown touchstart")},onView(storyId){$("[data-story-id='"+storyId+"']").unbind("click mousedown touchstart");$("#zuck-modal-slider-stories-main").unbind("mousedown");$("body").unbind("click mousedown touchstart");console.log("onView",storyId)},onEnd(storyId,callback){callback()
console.log("story end",storyId,callback)},onClose(storyId,callback){callback();console.log("story closed",storyId,callback)},onNavigateItem(storyId,nextStoryId,callback){callback();console.log("onNavigateItem",storyId,nextStoryId,callback)},onDataUpdate(currentState,callback){callback()}}};if(response.stories.length>0){stories_list($("#"+objs.story),response.stories);$("#"+objs.story).addClass("inited user-icon carousel snapgram");$("#"+objs.story).parent().removeClass("loading")}else{$("#"+objs.story).parent().remove()}
console.log(response)}};
ajax_hooks['search_store'] = {after:function(response,vars,form,objs){console.log(objs.typeahead)
objs.typeahead.$menu.removeClass("loading").removeClass("not-found");console.log(response);if(response){return objs.typeahead.render(response).show()}else{objs.typeahead.$menu.empty().addClass("not-found").html(objs.typeahead.highlighter("",""));return!1}}};
