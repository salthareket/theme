...
window.ajax_hooks['custom_modal'] = {required:["bootbox"],before:function(response,vars,form,objs){console.log(response);var className="modal-page loading "+(vars.class?vars.class:'');var scrollable=bool(vars.scrollable,!1);var close=bool(vars.close,!1);var centered=bool(vars.centered,!1);var animate=bool(vars.animate,!1);var backdrop=bool(vars.backdrop,!1);var dialog=bootbox.dialog({className:className,title:"<div></div>",message:'<div></div>',closeButton:close,size:!IsBlank(vars.size)?vars.size:'xl',scrollable:scrollable,centerVertical:centered,animate:animate,backdrop:backdrop,buttons:{},onHidden:function(e){response.abort()}});if(vars.fullscreen){dialog.find(".modal-dialog").addClass(vars.fullscreen)}
...
return response},after:function(response,vars,form,objs){console.log(vars);var modal=objs.modal;if(response.error){modal.addClass("remove-on-hidden").modal("hide");if(response.message){response_view(response)}
return!1}
...
modal.removeClass("loading");init_functions(response.data.plugins)}};
