...
obj.find(".item-total").html("SEARCHING...")},after:function(response,vars,form,objs){if(isset(objs[0])){var obj=$(objs[0])}else if(isset(objs.obj)){var obj=$(objs.obj)}
...
var url="";if(vars.url.indexOf("?")>-1){var url_parts=vars.url.split("?");if(!IsBlank(url_parts[1])){var url_json=url2json(vars.url);if(url_json){if(url_json.hasOwnProperty("paged")){}
url_parts[1]=json2url(url_json)}
var url=url_parts[0]+"page/"+vars.page+"/?"+url_parts[1]}else{var url=url_parts[0]+"page/"+vars.page+"/"}}else{var url=vars.url+"page/"+vars.page+"/"}
if(!IsBlank(url)){history.pushState(null,document.title,url)}
...
ajax_hooks['site_config'] = {init:function(){var query=new ajax_query();query.method="site_config";query.request()},after:function(response,vars,form){site_config=response;if(site_config.hasOwnProperty("nonce")){if(!IsBlank(site_config)){ajax_request_vars.ajax_nonce=site_config.nonce}}
if(site_config.hasOwnProperty("favorites")){var favorites=$.parseJSON(site_config.favorites);if(favorites.length>0){debugJS(favorites)
