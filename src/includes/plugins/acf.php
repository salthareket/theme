...
function acf_export_admin_page() {
    // `acf-field-group-category` taksonomisindeki kategorileri al
    ...
    if (empty($categories)) {
        echo '<div class="wrap"><h1>ACF Export</h1><p>No categories found in <strong>acf-field-group-category</strong>.</p></div>';
        return;
    }

    echo '<div class="wrap">';
    echo '<h1>ACF Export</h1>';
    echo '<p>Click on a category to export its ACF field groups as PHP.</p>';
    echo '<ul>';
    foreach ($categories as $category) {
        if ($category->count > 0) {
            $url = admin_url('admin-ajax.php?action=export_acf_field_group&category=' . urlencode($category->slug));
            echo '<li><a href="' . esc_url($url) . '" target="_blank">' . esc_html($category->name) . '</a></li>';
        }
    }
    echo '</ul>';
    echo '</div>';
}
add_action('acf/render_field/name=acf_export', 'acf_export_admin_page');
add_action('wp_ajax_export_acf_field_group', function () {
    if (!current_user_can('manage_options')) {
        wp_die('Unauthorized', 'Error', 403);
    }

    if (!isset($_GET['category']) || empty($_GET['category'])) {
        wp_die('Invalid category.', 'Error', 400);
    }

    $category = sanitize_text_field($_GET['category']);

    // ACF field gruplarını al
    $field_groups = acf_get_field_groups();

    if (empty($field_groups)) {
        wp_die('No field groups found.', 'Error', 404);
    }
    ...
    foreach ($field_groups as $field_group) {
        // Field grubu kategorisini kontrol et
        $categories = array_keys($field_group['acfe_categories'] ?? []);
        ...
        // Field grubu ve alanlarını alın
        $fields = acf_get_fields($field_group['key']);
        if (empty($fields)) {
            continue;
        }

        // Numeric index'i kaldırarak temiz dizi oluştur
        $clean_fields = array_map(function ($field) {
            return array_filter($field, function ($key) {
                return !is_numeric($key); // Sadece numeric olmayan keyleri tut
            }, ARRAY_FILTER_USE_KEY);
        }, $fields);

        // Field grubu array'ini güncelle
        $field_group['fields'] = $clean_fields;

        // ID'yi kaldırıyoruz çünkü gereksiz olabilir
        unset($field_group['ID']);

        $php_code .= "\tacf_add_local_field_group(" . var_export($field_group, true) . ");\n\n";
    }

    if (!$found) {
        wp_die('No field groups matched the category.', 'Error', 404);
    }
    ...
    header('Content-Disposition: attachment; filename="acf-field-groups-' . $category . '.php"');
    echo $php_code;
    exit;
});
