    ...
    public function plugin_settings(){
        ...
        foreach($this->rules["js"]["plugins"] as $key => $plugin){
            if(!$plugin["c"]){
                ...
                // required key'inden dependency'leri al (dependencies key'i de destekle)
                $raw_deps = [];
                if (!empty($plugin["required"]) && is_array($plugin["required"])) {
                    $raw_deps = $plugin["required"];
                } elseif (!empty($plugin["dependencies"]) && is_array($plugin["dependencies"])) {
                    $raw_deps = $plugin["dependencies"];
                }

                // Sadece c:true olan dependency'leri ekle — c:false olanlar zaten her sayfada yükleniyor
                $deps = [];
                foreach ($raw_deps as $dep) {
                    if (isset($this->rules["js"]["plugins"][$dep]) && !empty($this->rules["js"]["plugins"][$dep]["c"])) {
                        $deps[] = $dep;
                    }
                }
                $deps = array_values(array_unique($deps));

                // Class ve attrs arraylerini merge et
                $selectors = [];
                if (!empty($plugin["class"])) {
                    if (is_array($plugin["class"])) {
                        foreach ($plugin["class"] as $class_item) {
                            // Class'ın başında nokta yoksa ekle
                            $selectors[] = (strpos($class_item, '.') === 0) ? $class_item : '.' . $class_item;
                        }
                    } else {
                        // Class'ın başında nokta yoksa ekle
                        $selectors[] = (strpos($plugin["class"], '.') === 0) ? $plugin["class"] : '.' . $plugin["class"];
                    }
                }
                if (!empty($plugin["attrs"])) {
                    if (is_array($plugin["attrs"])) {
                        foreach ($plugin["attrs"] as $attr_item) {
                            // Attribute selector'ın köşeli parantez içinde olduğundan emin ol
                            if (strpos($attr_item, '[') !== 0 && strpos($attr_item, '.') !== 0 && strpos($attr_item, '#') !== 0) {
                                // data-toggle="confirm" -> [data-toggle="confirm"]
                                $selectors[] = '[' . $attr_item . ']';
                            } else {
                                $selectors[] = $attr_item;
                            }
                        ...
                        // Attribute selector'ın köşeli parantez içinde olduğundan emin ol
                        if (strpos($plugin["attrs"], '[') !== 0 && strpos($plugin["attrs"], '.') !== 0 && strpos($plugin["attrs"], '#') !== 0) {
                            // data-toggle="confirm" -> [data-toggle="confirm"]
                            $selectors[] = '[' . $plugin["attrs"] . ']';
                        } else {
                            $selectors[] = $plugin["attrs"];
                        }
                    }
                ...
                $js_list_conditional_set[$key] = [
                    ...
                    "selector" =>     !empty($selectors) ? $selectors : "",
                    "dependencies" => $deps,
                ];
            }
            $js_list_all[] = $key;
            ...
            foreach ($matches[3] as $key => $match) {
                if (substr($match, 0, 5) != "data:") {
                    ...
                    if (!preg_match('/\.[a-zA-Z0-9]+$/', $file)) {
                        ...
                        // Array key kontrolü ekle
                        if (isset($relative_path_parts[1])) {
