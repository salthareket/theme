        ...
        if ($rules["js"]["plugins"]) {
            ...
            foreach ($rules["js"]["plugins"] as $key => $item) {
                if($item["c"]){
                    ...
                    error_log("kaydedildi: ".$plugin_dir . $key . '.js');
                    ...
                    $plugin_min_files[] = $plugin_dir . $key . '.js';
            ...
            if($plugin_min_files){
                error_log("start to minify");
                error_log(json_encode($plugin_min_files));
                foreach($plugin_min_files as $key => $plugin_file){
                    if ($key == 0){
                        $minifier = new Minify\JS($plugin_file);
                    }else{
                        $minifier->add($plugin_file);
                    }
                }
                $minifier->minify($rules["config"]["min"] . 'plugins.min.js');
            }
            /*if($counter > 0){
